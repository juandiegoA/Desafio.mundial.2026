...
const std::array<uint8_t, 4>& Grupo::getIdsEquipos() const { return idsEquipos; }
uint8_t Grupo::getCantidadEquipos() const { return cantidadEquipos; }
const std::array<Partido, 6>& Grupo::getPartidos() const { return partidos; }
std::array<Partido, 6>& Grupo::getPartidos() { return partidos; }
...
bool Grupo::agregarEquipo(uint8_t equipoId) {
    if (cantidadEquipos >= 4) return false;
    ...
    for (uint8_t i = 0; i < 4; ++i) {
        for (uint8_t j = i + 1; j < 4; ++j) {
            partidos[cantidadPartidos].configurar(
                f, 0, 0, arbitros, Etapa::GRUPOS, idsEquipos[i], idsEquipos[j]
                );
            ++cantidadPartidos;
        }
    }
...
std::array<FilaTablaGrupo, 4> Grupo::construirTabla(const std::vector<Equipo>& equipos) const {
    ...
    std::sort(tabla.begin(), tabla.end(), [&equipos](const FilaTablaGrupo& a, const FilaTablaGrupo& b) {
